...
def get_bukov(json_bytes: bytes, lat: float, lon: float, sensors_profile:pl.DataFrame) -> pl.DataFrame:
...
if __name__ == "__main__":
    # Example usage
    lat = 49.0  # Example latitude
    lon = 16.0  # Example longitude
    sensors_profile = pl.DataFrame({
        "sen_lat": [49.0],
        "sen_lon": [16.0],
        "profile_number": ["T1"]
    })

    with open('inputs/T_stanoviste_123_from_2024.json', 'r') as file:
        data = file.read()

    bukov_data = get_bukov(data, lat, lon, sensors_profile)
    print(bukov_data)