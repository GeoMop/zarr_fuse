...
IN_JSON  = Path("bukov/T_123_partial.json")            # input JSON with old data
OUT_CSV  = Path("bukov/T_123_partial_normalized.csv")   # CSV output
...
IN_JSON_NEW  = Path("bukov/20250915T111522_824a7f3dc0ad.json")          # input JSON from Fiedler
OUT_CSV_NEW  = Path("bukov/from_fiedler_normalized.csv")   # CSV output
...
def normalize(filepath: Path, which: str):
    if which == "old":
        with filepath.open("r", encoding="utf-8") as f:
            data = json.load(f)
    else:
        data = read_new_fiedler_json(filepath)
...
def read_new_fiedler_json(filepath: Path) -> dict:
